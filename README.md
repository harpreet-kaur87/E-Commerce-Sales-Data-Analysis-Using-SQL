...
3. Find the count of customers from each region. Display the region name along with the total number of customers.
select region, count(*) count_of_customers from customers group by region order by count_of_customers desc;

4. Calculate the average quantity of products ordered by each customer.
